...
# Função sucessora (estratégia de exploração)
def DFS_limited(adj_list, start, goal, d):
    visited = [] # visitados
    fringe = [start] # fronteira (pilha com candidatos)
    path = [[start]] # path de resposta
    ...
    while fringe:
        ...
        distance = len(cur_path)

        # visita o nó
        if cur_node not in visited and distance <= d:
            visited.append(cur_node)
...
# Função sucessora (estratégia de exploração)
def IDFS(adj_list, start, goal, limit):
    visited = [] # visitados
    fringe = [(0, start)] # fronteira (pilha com candidatos) com prioridades
    path = [(0, [start])] # path de resposta

    for i in range(limit):
        response = DFS_limited(adj_list, start, goal, i)
        if response:
            return response
...
if __name__ == "__main__":
    print(DFS_limited(G0, "S", "g", 2))
    print(IDFS(G0, "S", "g", 2))

    print(DFS_limited(G0, "S", "g", 10))
    print(IDFS(G0, "S", "g", 10))
