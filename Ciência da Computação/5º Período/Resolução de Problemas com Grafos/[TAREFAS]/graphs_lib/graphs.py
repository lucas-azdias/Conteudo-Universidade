...
# ADD MEDIDAS DE ANALISE DE CAMINHOS (ECENTRICIDADE, DIAMETRO, RAIO E AVARAGE GEODESIC DISTANCE)
# ADD MEDIDES DE ANALISE DE CENTRALIDADE (e implementar uma função para calcular o graph com k-core)
# ADD METHOD UPDATE_NODE_DATA TO GRAPH -> update_node -> for leaf/composite (depading of the args)
...
_SEARCH_METHODS = Literal["iterative", "recursive"]  # Literal type for specifying search method
_COST_TYPES = Literal["minimum", "maximum"]  # Literal type for specifying cost type
...
class Graph:
    ...
    def __str__(self) -> str:
        ...
        """
        if not self.__list:  # Check if graph is empty
            return "<Empty graph>"
        
        text = ""
        for u in self.nodes():
            text += f"{u}: "
            for _, v, weight in self.out_edges(u):
    ...
    # Return a simplified string representation of the graph
    def __repr__(self) -> str:
        """
        Return a simplified string representation of the graph.

        Returns:
            str: Simplified string representation of the graph.
        """
        if not self.__list:  # Check if graph is empty
            return "<Empty graph>"
        
        return f"Graph[{', '.join(self.nodes())}]"
    ...
    # Perform Edmonds' Algorithm or Prim's Algorithm to get the minimum or the maximum spanning arborescence/tree
    def spanning_arborescence(self, u:str|None=None, cost:_COST_TYPES="minimum") -> Self:
        """
        Perform Edmonds' Algorithm or Prim's Algorithm to get the minimum or the maximum spanning arborescence/tree.

        Args:
            u (str | None, optional): The root of the arborescence/tree. Defaults to None.

        Returns:
            Self: Graph representing the minimum or the maximum spanning arborescence/tree.
        """
        # 1. If no valid node is passed, then calculates the root for the arborescence
        if not u or not self.has_node(u):
            # The root node is decided based on the node with the biggest sum of all outgoing edges weights
            nodes_out_edges_weights = [
                (
                    node,
                    reduce(lambda x, y: (None, None, x[2] + y[2]), self.out_edges(node))[2] if self.out_edges(node) else -np.inf
                ) for node in self.nodes()
            ]  # Calculates the sum of all outgoing edges weights for each node
            u = max(nodes_out_edges_weights, key=lambda x: x[1])[0]
        
        # New graph with the spanning arborescence/tree
        spanning_graph = Graph(is_directed=self.is_directed, is_weighted=self.is_weighted)

        # Edmonds' Algorithm (Spanning Arborescence)
        if self.is_directed:
            print("A FAZER")
            return
            # 2. Only leave the outgoing edges for the root node
            for _, node, weight in self.out_edges(u):
                spanning_graph.add_edge(u, node, weight)
            
            # 3. For each node, only leave the edge with the TODO *smallest*?? weight (except for the root node)
            for node in self.nodes():
                if node == u:
                    continue
                
                cur_node, _, weight = max(self.in_edges(node), key=lambda x: x[2])
                spanning_graph.add_edge(cur_node, node, weight)

            # 4. In a loop, for each cycle (strongly connected component), break it
            components_graph = spanning_graph.connected_components()
            while len(components_graph.nodes()) < len(spanning_graph.nodes()):
                # TODO NEED TO DIFERENCIATE A COMPOST NODE FROM A UNITARY NODE -> NEED TO IMPLEMENT NODE CLASS
                pass

            print(spanning_graph.connected_components().nodes())
        ...
        # Prim's Algorithm (Spanning Tree)
            ...
            for node in paths.keys():
                if paths[node][1]:
                    spanning_graph.add_edge(node, paths[node][1], paths[node][0])
