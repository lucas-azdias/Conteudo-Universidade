...
class adj_list_graph:
    ...
    def is_connected(self) -> bool:
        ...
        for node in self.nodes():
            if len(self.breadthfirst_search(node)) != self.get_order():
                return False
        return True
    ...
    def is_weakly_connected(self) -> bool:
        ...
        for node in self.nodes():
            if len(self.breadthfirst_search(node, ignore_direction=True)) != self.get_order():
                return False
        return True
    ...
    def dijkstra_all(self, u:str) -> dict[str, tuple[tuple[str], float]] | None:
        """
        Perform Dijkstra's algorithm to find the shortest path between u and all nodes.
    ...
    def connected_components(self) -> Self:
        ...
        stepmarkers = {node: [None, None] for node in graph.nodes()}
        ...
        def visit_marking(u:str, step:int) -> None:
            ...
            # Visits all outgoing nodes
            for out_node in graph.out_nodes(u):
        ...
        while finished:
            ...
            while pending:
                ...
                appendeds = []
                for out_node in graph.in_nodes(cur_node):
        ...
        for component in components:
            ...
            graph.add_node(component_node)  # Adds the new component node
            ...
            for node in component:
                ...
                for u, v, weight in edges:
                    if not u in component:
                        graph.add_edge(u, component_node, weight)
                    elif not v in component:
                        graph.add_edge(component_node, v, weight)
...
if __name__ == "__main__":
    ...
    g = adj_list_graph(is_directed=True)
    g.add_edge("A", "B")
    g.add_edge("A", "F")
    g.add_edge("B", "H")
    g.add_edge("D", "C")
