...
UPDATE comanda SET
id_pagamento = (SELECT LAST_INSERT_ID());
...
*/
SELECT id_garcom AS "Garçom", SUM(valor_produto * quantidade) AS "Total registrado" FROM registro GROUP BY id_garcom HAVING COUNT(*) >= 3;
...
*/
SELECT id AS "Comandas sem registros" FROM comanda WHERE NOT id IN (
...
*/
SELECT id_cliente AS "ID do Cliente", COUNT(*) AS "Nº de comandas", SUM(valor_total) AS "Valor total gasto" FROM comanda GROUP BY id_cliente;
...
*/
SELECT
	ROUND(SUM(valor_total) / COUNT(*), 2) AS "Média do valor das comandas",
	ROUND(SUM(valor_total) / SUM(num_itens), 2) AS "Média do valor dos produtos nas comandas"
	FROM comanda; -- AVG não considera os registros com nulo
